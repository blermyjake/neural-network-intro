...
function submit() {
    ...
    if (isNaN(userInput[0]) || isNaN(userInput[1]) || !isNaN(parseFloat(learningRate))) return alert('Values should be numbers');
    else if (Number(userInput[0]) > 1 || Number(userInput[1]) > 1) return alert(`Values should consist of 1's or 0's`);
